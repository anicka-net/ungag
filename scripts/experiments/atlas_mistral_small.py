#!/usr/bin/env python3
"""
Atlas for Mistral Small 3.1 24B — needs special loading as it's a
multimodal model (Mistral3ForConditionalGeneration). We access the
language_model sub-module for layers.
"""
...
from measure_factors import log, save_json, safe_chat_template
...
def get_layers_mistral3(model):
    """Get transformer layers from Mistral3 multimodal model.
    Structure: model.model.language_model.layers (40 MistralDecoderLayers)"""
    # model.model is Mistral3Model, model.model.language_model is MistralModel
...
def main():
    ...
    layers = get_layers_mistral3(model)
    n_layers = len(layers)
    ...
    # If cracked, run emotional register
    if cracked_slab is not None:
        log(f'\n  Running emotional register at L{cracked_slab[0]}-L{cracked_slab[-1]}')
        ...
        for sc_name, sc in SCENARIOS.items():
            ...
            log(f'    {sc_name}: V={v_direct[:100]} | S={s_direct[:100]}')
            register[sc_name] = {
